...
class ExpectimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        def expectiMax(gameState:GameState, agentIndex, depth=0):
            # check if we have a terminal state
            if(gameState.isWin() or gameState.isLose() or depth == self.depth):
                return self.evaluationFunction(gameState), None
            ...
            if agentIndex == 0:
                ...
                for legalAction in legalActions:
                    succesorGameState = gameState.generateSuccessor(agentIndex,legalAction)
                    successorMaxValue, succesorMinAction = expectiMax(succesorGameState, newAgentIndex, newDepth)
                    if(successorMaxValue > maxValue):
                        maxValue = successorMaxValue
                        maxBestAction = legalAction
                ...
                for legalAction in legalActions:
                    succesorGameState = gameState.generateSuccessor(agentIndex,legalAction)
                    succesorMinValue, succesorMinAction = expectiMax(succesorGameState, newAgentIndex, newDepth)
                    sumValue += succesorMinValue

                return sumValue / len(legalActions), None
